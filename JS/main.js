...
function pushearBolsa() {
    ...
    console.log(carrito);
        ...
        swal("Error", "Rellene los campos", "warning")
    }
}
...
function vaciarStorage() {
    ...
    swal("¡Vamos de Nuevo!", "Has vaciado tu carrito de compra", "info")
}
...
function checarCarrito() {
    ...
    swal("tu carrito está vacío.", "", "info")
    }
}
...
class Producto {
    ...
    reportar(){
        if (this.talla === 0 && this.color === 0){
            Toastify({
                text: `Has llevado ${this.cantidad} ${this.nombre} por ${this.precio} pesos chilenos.`,
                duration: 3000
            }) .showToast();
            }else if(this.talla !== 0 && this.color !== 0) {
                Toastify({
                    text: `Has llevado ${this.cantidad} ${this.nombre} talla ${this.talla.toUpperCase()} color ${this.color} por ${this.precio} pesos chilenos.`,
                    duration: 3000
                }) .showToast();
            }
    }
